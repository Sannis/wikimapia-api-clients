...
function standardRequestCallback(format, callback) {
  return function (err, response, body) {
    if (err) {
      ...
      if (format === 'json') {
        // Hack due to Wikimapia API JSON stringifier bug
        body = body.replace(/"polygon":".*?([^\\])"/, '"polygon":""');
 ...
 * @param callback
 */
Wikimapia.prototype.box = function (params, callback) {
  if (typeof callback !== 'function') {
    throw new TypeError('Wikimapia.box second argument must be a callback function');
  }

  request.get(
    { url: makeRequestUrl(this.options, params, 'box') },
